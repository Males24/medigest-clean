...
@section('title', __('messages.consultas.create_title').' - Médico')
...
@push('head')
  {{-- Endpoint de slots da API interna --}}
...
<div class="max-w-[1430px] mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-10"
     data-page="wizard-consulta-medico">
  ...
  <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-6 sm:mb-8">
    <div>
      <h1 class="text-2xl sm:text-3xl font-semibold tracking-tight text-gray-900">
        @lang('messages.consultas.create_title')
      </h1>
  ...
  <form id="wizardForm" method="POST" action="{{ route('medico.consultas.store') }}"
    ...
    <div class="p-6 sm:p-8 space-y-8">
      ...
      <section class="wizard-step" data-step="0">
        ...
        <select name="paciente_id" id="paciente_id" data-mg-select
      ...
      {{-- 2: Médico (pré-selecionado; UI bloqueada, mas valor será enviado) --}}
      <section class="wizard-step hidden" data-step="2">
        ...
        <select name="medico_id" id="medico_id" data-mg-select
          ...
          @foreach($medicos as $m)
            <option value="{{ $m->id }}" selected>{{ $m->name }} — {{ $m->email ?? '' }}</option>
          @endforeach
        </select>
      ...
      <section class="wizard-step hidden" data-step="5">
        ...
        <div class="relative">
          ...
          <textarea id="motivo" name="motivo" rows="5" maxlength="400"
      ...
      <section class="wizard-step hidden" data-step="6">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
          {{-- cartões de revisão (paciente, especialidade, médico, etc.) --}}
          ...
          <div class="rounded-xl border border-gray-200 bg-white p-4 shadow-sm flex items-start gap-3">
            <div class="w-9 h-9 rounded-lg bg-emerald-50 ring-1 ring-emerald-100 flex items-center justify-center shrink-0">
              <svg class="w-5 h-5 text-emerald-700" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"><path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 19V4a1 1 0 0 1 1-1h12a1 1 0 0 1 1 1v13H7a2 2 0 0 0-2 2Zm0 0a2 2 0 0 0 2 2h12M9 3v14m7 0v4"/></svg>
            </div>
            <div class="min-w-0">
...
{{-- JS exclusivo do médico --}}
@vite('resources/js/pages/medico/consultas/consultas-medico-create.js')
@endsection

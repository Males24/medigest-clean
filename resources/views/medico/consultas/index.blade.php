...
@section('title', __('messages.consultas.list_title').' - Médico')
...
<div class="max-w-[1430px] mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-10 space-y-6">
  ...
  <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
    <div>
      <h1 class="text-2xl sm:text-3xl font-semibold tracking-tight text-gray-900">
        @lang('messages.consultas.list_title')
      </h1>
      ...
      <div class="flex-1">
        <div class="group relative rounded-2xl ring-1 ring-transparent transition
          ...
          <input type="search" name="q" value="{{ request('q') }}"
                 ...
                 placeholder="Pesquisar por paciente, especialidade ou estado…">
  ...
  @endif
  @if($errors->any())
    <div class="rounded-xl px-4 py-3 bg-rose-50 text-rose-800 ring-1 ring-rose-200/60">
      {{ $errors->first() }}
    </div>
  @endif
  ...
  <div class="bg-white rounded-2xl shadow-sm ring-1 ring-gray-200/60 overflow-hidden">
            ...
            <tr class="border-t border-gray-100 hover:bg-gray-50">
              ...
              <td class="px-6 py-3 text-right">
                ...
                <button type="button"
                  ...
                  data-confirm-url="{{ $canReview ? route('medico.consultas.confirmar', $consulta) : '' }}"
                  data-reject-url="{{ $canReview ? route('medico.consultas.rejeitar', $consulta) : '' }}">
    ...
    <nav class="flex flex-col items-start gap-3 md:flex-row md:items-center md:justify-between p-4 border-t border-gray-100 bg-white"
      ...
      <ul class="inline-flex items-stretch -space-x-px">
        <li>
          @if($prevUrl)
            <a href="{{ $prevUrl }}" class="flex items-center justify-center h-full py-1.5 px-3 ml-0 text-gray-600 bg-white rounded-l-lg border border-gray-300 hover:bg-gray-100 hover:text-gray-700">
        ...
        @foreach($pages as $p)
          ...
          @if($p == $current)
            <li>
              <span aria-current="page" class="z-10 flex items-center justify-center px-3 py-2 text-sm leading-tight border text-home-medigest bg-emerald-50 border-emerald-200 hover:bg-emerald-100">
                {{ $p }}
              </span>
            </li>
          @else
            <li>
              <a href="{{ request()->fullUrlWithQuery(['page'=>$p]) }}" class="flex items-center justify-center px-3 py-2 text-sm leading-tight text-gray-600 bg-white border border-gray-300 hover:bg-gray-100 hover:text-gray-800">
                {{ $p }}
              </a>
            </li>
          @endif
        ...
        <li>
          @if($nextUrl)
            <a href="{{ $nextUrl }}" class="flex items-center justify-center h-full py-1.5 px-3 text-gray-600 bg-white rounded-r-lg border border-gray-300 hover:bg-gray-100 hover:text-gray-700">
...
@vite('resources/js/pages/medico/consultas/consultas-medico-index-modal.js')
@vite('resources/js/pages/medico/consultas/consultas-medico-index-dropdown.js')
@vite('resources/js/pages/medico/consultas/consultas-medico-index-filters.js')
@endsection

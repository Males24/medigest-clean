...
<body class="bg-gray-100 text-gray-900 antialiased">
  ...
  {{-- HEADER --}}
  <header class="relative bg-white border-b border-gray-200 px-4 sm:px-6 lg:px-8 py-3 sticky top-0 z-40">

    {{-- GRUPO ESQUERDO (fixo): hambúrguer + logo, ancorados ao header --}}
    <div class="absolute left-3 sm:left-4 top-1/2 -translate-y-1/2 flex items-center gap-3">
      <button id="sidebar-toggle"
              type="button"
              class="inline-flex items-center justify-center rounded-lg p-2
                     hover:bg-gray-100 text-gray-700
                     border border-white/50 ring-1 ring-gray-200/80
                     shadow-[inset_-1px_-1px_4px_rgba(255,255,255,0.95),inset_3px_3px_8px_rgba(15,23,42,0.10)]
                     focus:outline-none focus-visible:ring-2 focus-visible:ring-emerald-500/35"
              aria-controls="layout-sidebar" aria-pressed="false">
      ...
      <a href="{{ route('home') }}" class="inline-flex items-center shrink-0 select-none"
         aria-label="{{ __('messages.app.name') }}">
        <img src="{{ asset('/Logo_Preto.svg') }}"
             alt="{{ __('messages.app.name') }}"
             class="h-8 sm:h-10 w-auto" loading="eager" fetchpriority="high">
      </a>
    </div>

    {{-- Contentor centrado (direita): reservamos espaço à esquerda para o grupo fixo --}}
    <div class="max-w-[1430px] mx-auto flex items-center justify-end gap-3
                pl-[9rem] sm:pl-[10rem]">
      ...
      @auth
        ...
        <div class="relative">
          <button id="user-menu-button" type="button" aria-haspopup="menu" aria-expanded="false"
            ...
            <svg id="user-chevron" class="w-3.5 h-3.5 ms-0.5 text-gray-600 transition-transform duration-150"
  ...
  <div class="relative">
    <div class="flex min-h-[calc(100vh-64px)]">
      ...
      {{-- overlay (mobile) --}}
      <div id="sidebar-scrim" class="lg:hidden fixed left-0 right-0 z-20" aria-hidden="true"></div>

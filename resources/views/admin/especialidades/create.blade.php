...
@section('title', __('messages.specialties.create'))
...
<div class="max-w-[1430px] mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-10">
  ...
  <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-6">
    <div>
      <h1 class="text-2xl sm:text-3xl font-semibold tracking-tight text-gray-900">@lang('messages.specialties.new')</h1>
      <p class="text-sm text-gray-500 mt-1">@lang('messages.specialties.new_subtitle')</p>
    </div>
  ...
  <form action="{{ route('admin.especialidades.store') }}" method="POST" enctype="multipart/form-data"
    ...
    <div class="grid sm:grid-cols-2 gap-4">
      <div>
        ...
        <p class="text-xs text-gray-500 mt-1">Opcional. Formatos: jpg, jpeg, png, webp, avif. Máx: 4MB.</p>
      ...
      <div>
        ...
        <input type="text" id="cover_path" name="cover_path" value="{{ old('cover_path') }}"
        ...
        <p class="text-xs text-gray-500 mt-1">Opcional. Se carregar um ficheiro, o upload tem prioridade sobre este campo.</p>
        @error('cover_path') <p class="text-sm text-red-600 mt-1">{{ $message }}</p> @enderror
      </div>
    </div>

    <div class="flex items-center gap-3 pt-2">
      <button class="px-5 py-2.5 rounded-xl bg-home-medigest text-white hover:bg-home-medigest-hover">@lang('messages.actions.confirm')</button>

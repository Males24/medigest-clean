...
<div class="max-w-[1430px] mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-10 space-y-6">
  ...
  @php
    $from    = $medicos->firstItem() ?? 0;
    $to      = $medicos->lastItem()  ?? $medicos->count();
    $total   = $medicos->total();
    $current = $medicos->currentPage();
    $last    = $medicos->lastPage();
    $window  = 1;
...
@vite('resources/js/pages/admin/medicos/medicos-admin-index-dropdown.js')
@vite('resources/js/pages/admin/medicos/medicos-admin-index-modal.js')
@endsection

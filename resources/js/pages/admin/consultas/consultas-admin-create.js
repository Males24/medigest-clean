document.addEventListener('DOMContentLoaded', () => {
  ...
  // devolve o índice máximo que pode ser clicado (Descrição é opcional)
  function maxUnlocked(){
    ...
    if (!(dataEl.value && horaSel.value)) return max; max = 5; // descrição
    // todos os obrigatórios estão ok -> revisão
    return 6;
  }
  ...
  espSel?.addEventListener('change', async () => {
    ...
    try {
      ...
      if (!Array.isArray(list) || list.length === 0) {
        ...
        window.__refreshMgSelect?.(medicoSel);
        updateClickable();
        return;
      }
      const ph = document.createElement('option');
      ph.value = ''; ph.textContent = '— Seleciona —'; medicoSel.appendChild(ph);
      list.forEach(m => { const o = document.createElement('option'); o.value = m.id; o.textContent = `${m.name} — ${m.email ?? ''}`; medicoSel.appendChild(o); });
      window.__refreshMgSelect?.(medicoSel);
    } catch (e) {
